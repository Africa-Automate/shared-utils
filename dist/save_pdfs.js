 ...
 */
function savePDFs(base64Images, path, name) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const imageBuffers = base64Images.map((base64Image) => Buffer.from(base64Image, "base64"));
        // Create a new PDF document
        const pdfDoc = yield pdf_lib_1.PDFDocument.create();
        for (const imageBuffer of imageBuffers) {
            const image = yield pdfDoc.embedJpg(imageBuffer);
        ...
        const pdfFileName = `Shop-Documents/${path}/${name}_${Date.now()}.pdf`;
        const pdfFile = firebase_1.storage.bucket("informal-traders-africa").file(pdfFileName);
        // Save the PDF to Firebase Storage
        yield pdfFile.save(Buffer.from(pdfBytes), {
            contentType: "application/pdf",
        });
        yield pdfFile.makePublic();
